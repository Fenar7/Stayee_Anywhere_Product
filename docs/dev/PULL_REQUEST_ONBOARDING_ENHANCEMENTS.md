...
- **Handcrafted Apple & Linear WhatsApp Dispatch Modal:** Completely redesigned the auto-dispatch modal to eliminate AI design tropes (bright green `#25D366` buttons, text `WA` circle avatars, and heavy drop shadows). Replaced with a 1px precision bordered surface (`backdrop-blur-md`), split credential cards with 1-click `Copy Link` & `Copy Password` pills, dark monospace message drawer, and a high-contrast dark CTA (`Send via WhatsApp →`) featuring an animated live status indicator (`●`).
