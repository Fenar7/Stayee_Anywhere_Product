...
- **Dual-Layer Image Compression & S3 Storage Pipeline (`lib/image/client-compress.ts`, `lib/image/index.ts`, `register/route.ts`):**
  - **Client-Side Pre-Compression:** Created a browser canvas pre-compression utility (`compressImageInBrowser`) that resamples high-resolution camera uploads down to 1000px × 1000px (JPEG quality 0.8) directly in the browser before network transmission.
  - **Server-Side `sharp` C++ Engine Optimization:** Server processes uploaded profile photos with `sharp`, stripping EXIF metadata, constraining resolution to 1000px max, and encoding to progressive JPEG (quality 70%–75%).
  - **Expected Storage Size Reduction:** Reduces raw mobile photos (3.5MB – 8MB) down to **~40 KB – 90 KB** (98%+ storage savings), ensuring lightning-fast dashboard avatar loading and reduced S3 storage costs.
